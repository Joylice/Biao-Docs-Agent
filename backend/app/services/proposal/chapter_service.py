...
def flatten_sections(sections: list[Any]) -> list[str]:
...
def _numbered_tree(nodes: list[Any], prefix: str, out: list[tuple[str, str]]) -> None:
...
def split_chapter_to_sections(
    content_md: str, sections_tree: list[Any], chapter_no: str
) -> list[dict[str, str]]:
    """按大纲嵌套树将整章正文切分为子节片段.
...
async def generate_chapter(
    ...
    supplement_points: list[dict[str, Any]] | None = None,
    benchmark_high_risk: list[dict[str, Any]] | None = None,
    ...
    if not context:
        try:
            ...
            # RAG 失败时降级为无上下文（观测：生成质量可能下降）
            import logging as _logging

            _logging.getLogger(__name__).warning(
                "章节生成 RAG 检索失败，降级为无上下文 project_id=%s", project_id, exc_info=True
            )
            context = ""
...
async def upsert_section(
    ...
    content_dsl: dict[str, Any] | None = None,
) -> None:
    """保存章节到 proposal_sections（upsert）；citations 为引用溯源元数据.

    content_dsl 为结构化内容 DSL JSON（Phase 2 新增）；为 None 时不覆盖已有值。
    """
    ...
    if not section:
        ...
        if content_dsl is not None:
            section.content_dsl = content_dsl
        db.add(section)
    else:
        section.title = title
...
async def persist_chapter_content(
    ...
    Phase 2：Markdown → DSL 解析后，章级行同时写 content_dsl。
    """
    from app.services.content.dsl_parser import md_to_dsl

    dsl_json = md_to_dsl(content).model_dump()
