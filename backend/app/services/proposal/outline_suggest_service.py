...
def _encode_suggestion_id(suggestion: dict[str, Any]) -> str:
...
def _decode_suggestion_id(suggestion_id: str) -> dict[str, Any] | None:
...
def _covered_clauses(outline: list[dict[str, Any]]) -> set[str]:
    """汇总大纲已覆盖的评分点子句号."""
    covered: set[str] = set()
...
def _rule_suggestions(
    score_points: list[dict[str, Any]], outline: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def _finalize_llm_suggestion(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _mock_fallback_suggestions(score_points: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
async def build_outline_suggestions(
    score_points: list[dict[str, Any]], outline: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def apply_outline_suggestions(
    outline: list[dict[str, Any]], adopted: list[str]
) -> list[dict[str, Any]]:
    ...
    未知/非法 id 忽略。
    """
    result: list[dict[str, Any]] = [dict(c) for c in outline]
