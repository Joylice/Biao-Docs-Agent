...
def build_format_spec(format_requirements: list[dict[str, Any]] | None) -> FormatSpec:
...
def apply_override(spec: FormatSpec, override: dict[str, Any] | None) -> FormatSpec:
    """字段级覆盖：仅覆盖 override 中非 None 的字段，未传字段保留原值.

    override 支持的 key 与 FormatSpec 字段一致：
    body_font / body_size_pt / heading_size_pt / line_spacing /
    line_spacing_fixed_pt / margins_cm（合并，不整体替换）
    """
    if not override:
        return spec

    if override.get("body_font") is not None:
        spec.body_font = override["body_font"]
    if override.get("body_size_pt") is not None:
        spec.body_size_pt = float(override["body_size_pt"])
    if override.get("heading_size_pt") is not None:
        spec.heading_size_pt = float(override["heading_size_pt"])
    if override.get("line_spacing") is not None:
        spec.line_spacing = float(override["line_spacing"])
        # 倍数行距与固定值互斥
        spec.line_spacing_fixed_pt = None
    if override.get("line_spacing_fixed_pt") is not None:
        spec.line_spacing_fixed_pt = float(override["line_spacing_fixed_pt"])
        spec.line_spacing = None

    margins_override = override.get("margins_cm")
    if isinstance(margins_override, dict):
        for side in ("top", "bottom", "left", "right"):
            val = margins_override.get(side)
            if val is not None:
                spec.margins_cm[side] = float(val)
