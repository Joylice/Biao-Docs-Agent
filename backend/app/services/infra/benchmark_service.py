...
MOCK_HIGH_RISK_SAMPLE: list[dict[str, Any]] = [
    {
        "clause_no": "MOCK-HR",
...
async def _retrieve(db: AsyncSession, project_id: uuid.UUID, query: str) -> list[ChunkResult]:
...
async def build_benchmark(db: AsyncSession, project_id: uuid.UUID) -> list[dict[str, Any]]:
...
async def load_high_risk_points(db: AsyncSession, project_id: uuid.UUID) -> list[dict[str, Any]]:
