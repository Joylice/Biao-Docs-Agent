...
async def save_section_edit(
    ...
    与自动重写的差异：不经 LLM，内容来自人工编辑；落库 status=review
...
async def sync_approved_chapter(
    ...
    if is_subsection:
        ...
        await _upsert_section(db, str(project_id), chapter_no, title, content, status="approved")
        sec_result = await db.execute(
            select(ProposalSection).where(
                ProposalSection.project_id == uuid.UUID(str(project_id)),
                ProposalSection.section_id.startswith(f"{parent_no}."),
...
async def export_workflow(
    project_id: uuid.UUID | str, export_options: dict[str, Any] | None = None
) -> dict[str, Any]:
    """导出 Word — 复用图内 export 节点（export_to_word + 落库 + 事件），结果回写 state.

    export_options 由前端传递（格式覆盖、导出范围等），透传给 export_node。
    """
    ...
    # 透传前端导出选项到 export_node
    if export_options:
        values["export_options"] = export_options
    # values 是运行时拼装的 state 增量（dict）；落到 export_node 的 BidState 边界
    updates = await export_node(cast("BidState", values))
