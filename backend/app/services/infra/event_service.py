...
# 注：redis 5.3 的模块级 `redis.asyncio.from_url` 是**未注解**的转发包装器
# （redis/asyncio/utils.py: `def from_url(url, **kwargs)` → `Redis.from_url`），
# mypy strict 会对该调用报 no-untyped-call。此处刻意保留模块级调用（而非直接调
# `Redis.from_url`），因为它是单元测试的既定 mock 点（tests/unit/test_event_service.py
# patch "redis.asyncio.from_url"）；直接改调用路径会让 mock 失效、静默走真实 Redis，
# 属"非纯类型的运行期行为变更"。故按 api/websocket.py 对 `pubsub.aclose()` 的既有做法
# 定向忽略该错误。
...
async def subscribe_events(project_id: str) -> tuple[Redis, PubSub]:
...
async def subscribe_user_events(user_id: str) -> tuple[Redis, PubSub]:
