...
async def delete_project(
    ...
    if project.owner_id != operator_id:
        ...
        user = user_result.scalar_one_or_none()
        is_admin = user is not None and await has_permission(db, user, "system:manage")
        if not is_admin:
...
async def _delete_workflow_checkpoints(db: AsyncSession, thread_id: str) -> None:
    ...
    try:
        ...
        # checkpointer 未初始化由 get_saver() 返回 None 表达（不进本分支）；
        # 能进到这里的是真实清理失败，静默会残留孤儿 checkpoint（P1-9）
        logger.warning("checkpoint 清理失败 thread_id=%s", thread_id, exc_info=True)
