...
async def _load_outline(db: AsyncSession, project_id: uuid.UUID) -> list[dict[str, Any]]:
...
async def update_chapter_content(
    ...
    """保存章节内容（Markdown + HTML + DSL 三字段；仅 flush 不 commit）.

    Phase 3：content_dsl 为结构化真源，为 None 时不覆盖已有值。
    """
    assignment.content = content
    assignment.content_html = content_html
    if content_dsl is not None:
        assignment.content_dsl = content_dsl
    await db.flush()
