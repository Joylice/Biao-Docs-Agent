...
def build_markdown_source(
    project_name: str, outline: list[dict[str, Any]], chapters: dict[str, str]
) -> str:
...
async def _latest_format_requirements(
...
async def create_snapshot(
    ...
    outline: list[dict[str, Any]] = values.get("outline", []) or []
    ...
    # Phase 2：快照追加 chapters_dsl（结构化 DSL 树，block 级回滚数据源）
    chapters_dsl: dict[str, dict[str, Any]] = {}
    try:
        from app.services.content.dsl_parser import md_to_dsl

        for ch_no, ch_md in chapters.items():
            chapters_dsl[str(ch_no)] = md_to_dsl(ch_md).model_dump()
    except Exception:
        # DSL 解析失败不阻塞快照（可观测：快照将缺失 chapters_dsl）
        logger.warning("快照 chapters_dsl 解析失败，本快照无 DSL 数据", exc_info=True)
    ...
    record = ProposalVersion(
        ...
        # Phase 2：追加 chapters_dsl（结构化 DSL 树）
        snapshot_json={
            "outline": outline,
            "chapters": chapters,
            "chapters_dsl": chapters_dsl,
        },
    )
    db.add(record)
    await db.flush()
...
async def rollback_version(
    ...
    Phase 2：同时回写 content_dsl（结构化 DSL 树，block 级回滚）。
    返回恢复的章节数；无结构化快照抛 ValidationError。
    """
    data = version.snapshot_json or {}
    chapters: dict[str, str] = data.get("chapters") or {}
    chapters_dsl: dict[str, dict[str, Any]] = data.get("chapters_dsl") or {}
    outline: list[dict[str, Any]] = data.get("outline") or []
    if not chapters:
    ...
    for chapter_no, content in chapters.items():
        ...
        dsl = chapters_dsl.get(chapter_no)
        result = await db.execute(
...
async def list_versions(db: AsyncSession, project_id: uuid.UUID) -> list[dict[str, Any]]:
