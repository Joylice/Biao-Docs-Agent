...
async def enqueue_reindex_all() -> bool:
    """入队全量重建索引任务（worker.tasks.task_reindex_all）.

    无 project_id/doc_id 参数（全量扫描），Redis 不可用时返回 False。
    """
    try:
        from arq import create_pool
        ...
        pool = await create_pool(parse_redis_url(settings.redis_url))
        try:
            await pool.enqueue_job("task_reindex_all")
        finally:
            await pool.aclose()
        return True
    except Exception as e:
        logger.warning("arq 入队失败（降级，不阻断主流程）: job=task_reindex_all, err=%s", e)
        return False
