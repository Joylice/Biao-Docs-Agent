...
async def _upsert_skeleton(db: AsyncSession, project_id: str, tree: list[dict[str, Any]]) -> None:
...
async def generate_outline_node(state: BidState) -> dict[str, Any]:
    """节点：生成大纲 — 基于 state 中的评分点，落库 proposal_skeletons.

    2026-08-26 单一数据源重构：generate_outline 改纯 state 读，regenerate 路径
    经独立 refresh_context 节点从 DB 刷新源数据写回 state，本节点不再直接读 DB。
    P3：技术需求已移除，大纲仅以评分点为核心纲要组织章节。
    """
...
async def confirm_outline_node(state: BidState) -> dict[str, Any]:
    ...
    while True:
        ...
        start_generation = (
            decision.get("start_generation", True) if isinstance(decision, dict) else True
        )
        ...
        updates: dict[str, Any] = {
            ...
            "regenerate_requested": False,
            "web_search_enabled": (
                decision.get("web_search_enabled", False) if isinstance(decision, dict) else False
            ),
        }
...
async def _clear_outline_draft(db: AsyncSession, project_id: str) -> None:
    """确认大纲后清除二次编辑草稿（幂等）."""
    result = await db.execute(
        select(ProposalSkeleton).where(ProposalSkeleton.project_id == uuid.UUID(project_id))
    )
