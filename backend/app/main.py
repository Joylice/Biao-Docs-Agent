...
# Windows 上 psycopg3 异步驱动不兼容 ProactorEventLoop，必须切换到 SelectorEventLoop。
# 在模块导入最早期执行，确保在 psycopg 创建连接池前生效。
...
async def biz_error_handler(_request: Request, exc: BizError) -> JSONResponse:
    ...
    if exc.code == 4001:
        ...
        # 指定节点重跑冲突：运行中/不在白名单（409 Conflict）
        http_status = 409
    elif exc.code >= 5000:
        http_status = 500
    return JSONResponse(
...
app.include_router(usage.router, prefix=f"{settings.api_prefix}", tags=["LLM 用量"])
