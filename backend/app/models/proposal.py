...
class ProposalSkeleton(Base):
    ...
    # 实际恒为大纲嵌套树**列表**：outline 节点写 list、division_service 读取时还有
    # isinstance(tree, list) 守卫。历史迁移 0003 的 server_default="{}" 仅对绕过 ORM
    # 的裸插生效，ORM 恒写 list，故无需新增迁移。
    tree: Mapped[list[dict[str, Any]]] = mapped_column(JSON, nullable=False, default=list)
    # 大纲二次编辑草稿（确认前防丢失；确认成功后清空）：{"outline": [...], "mounted_doc_ids": [...]}
    draft: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True, default=None)
...
class ProposalSection(Base):
    ...
    # 结构化内容 DSL JSON（content_dsl 为真源；NULL 时惰性迁移自 content_md）
    content_dsl: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True, default=None)
    # draft|generating|review|approved
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="draft")
    # [{chunk_id,doc_title,page_no}]
    # 实际恒为引用列表 [{chunk_id, doc_title, page_no}]：write_node 落库、review/export
    # 读取均按 list 处理（review.py 有 isinstance(cits, list) 守卫）。历史迁移 0003 的
    # server_default="{}" 仅对绕过 ORM 的裸插生效，ORM 恒写 list，故无需新增迁移。
...
class ChapterAssignment(Base):
    ...
    # 结构化内容 DSL JSON（content_dsl 为真源；NULL 时惰性迁移自 content）
    content_dsl: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True, default=None)
