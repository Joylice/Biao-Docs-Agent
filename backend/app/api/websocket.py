...
async def _current_status(project_id: str) -> dict[str, Any]:
...
async def _forward_pubsub(websocket: WebSocket, client: Redis, pubsub: PubSub) -> None:
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            # redis-py 的 PubSub.aclose 未带注解（Redis.aclose 有），非本仓可控
