...
class ChapterContentBody(BaseModel):
    """章节内容保存请求体：content 可空串（已清空），content_html 可空，content_dsl 可空."""

    content: str = Field(..., min_length=0)
    content_html: str | None = None
    content_dsl: dict[str, Any] | None = None  # Phase 3：结构化 DSL JSON（真源）
...
async def get_chapter_content(
    ...
    return success(
        data={
            ...
            "content_dsl": assignment.content_dsl,
        }
    )
...
async def save_chapter_content(
    ...
    状态锁定（2026-09-03 产品确认）：已提审（submitted）/ 已通过（approved）章节内容锁定，
    不可再编辑；仅被打回（rejected）后由负责人修改并重新提交。
    """
    await _check_project_member(db, project_id, user_id)
    assignment = await division_service.get_assignment_by_chapter(db, project_id, chapter_no)
    if assignment is None:
    ...
    if assignment.status in ("submitted", "approved"):
        raise ForbiddenError("章节已提交审核或已通过，内容已锁定，不可编辑")
    await division_service.update_chapter_content(
        db, assignment, body.content, body.content_html, body.content_dsl
    )
    await audit.record(
        db,
        user_id,
