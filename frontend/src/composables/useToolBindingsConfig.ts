/**
 * useToolBindingsConfig：外部工具 ↔ 阶段绑定（原「技能」分类）逻辑模型.
 *
 * 🔴 S4 改名说明：本文件原名 `useSkillsConfig.ts`。SKILL.md 契约体系（S4）
 * 引入后，「skill」这个名字被让给行为准则资产（`api/skills.ts` +
 * `SkillPanel.vue`），本模块管理的其实是「外部工具绑定到编制阶段」，故整体
 * 更名为 `useToolBindingsConfig` —— 语义与实体对齐，避免与真 skill 混淆。
 *
 * 「外部工具」= 流水线阶段的业务视图（旧 SkillsView 的阶段绑定管理部分迁移）：
 * 列表来自 GET /settings/routes（8 阶段的名称/启用态），启停 =
 * PUT /settings/routes/{stage_key} { enabled }（后端无路由乐观锁，即时保存）；
 * 阶段绑定 = 该阶段绑定的外部工具（数据源为 useExternalToolsConfig 的 tools +
 * 会话内 bindings，bind/unbind 走 /settings/external-tools/{id}/bind 端点）。
 *
 * 实现为「依赖注入工厂 + 模块级单例」（单测经 createToolBindingsConfig 注入）。
 */
...
/**
 * 工具（外部工具）视图行：ToolBindingsPanel 渲染与单测共用.
 *
...
export interface ToolBindingsConfigDeps {
...
export interface ToolBindingsConfigApi {
  loading: Ref<boolean>
...
export function createToolBindingsConfig(deps: ToolBindingsConfigDeps): ToolBindingsConfigApi {
  const { getRoutes, updateRoute, getTools, bindTool, unbindTool, notifyError, notifySuccess } =
    deps
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      notifyError(getApiErrorMessage(error, '加载外部工具配置失败'))
    } finally {
      loading.value = false
    }
  }
...
let singleton: ToolBindingsConfigApi | null = null

export function useToolBindingsConfig(): ToolBindingsConfigApi {
  if (!singleton) {
    singleton = createToolBindingsConfig({
      getRoutes,
      updateRoute,
